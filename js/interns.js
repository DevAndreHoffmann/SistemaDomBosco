...
import { showClientDetails } from './clients.js'; // Import showClientDetails to allow navigation
...
export function renderInternList(filter = '') {
    ...
    filteredInterns.forEach(intern => {
        ...
        // Count clients *currently assigned* to this intern
        ...
        card.innerHTML = `
            ...
            <p><strong>Pacientes Atribuídos:</strong> ${clientsAssignedCount}</p>
        `;
...
export function showInternDetails(internId) {
    ...
    // Filter clients *currently assigned* to this intern
...
function renderInternClientList(clients) {
    ...
    if (clients.length === 0) {
        internPatientsList.innerHTML = '<p>Este estagiário ainda não tem pacientes atribuídos.</p>';
        return;
    }
    ...
    clients.forEach(client => {
        ...
        card.addEventListener('click', () => {
            ...
            showClientDetails(client.id); // Use the imported showClientDetails
        });
...
export function deleteIntern(internId) {
    ...
    // NEW LOGIC: Unassign clients that were assigned to this intern
    db.clients.forEach(client => {
        if (client.assignedInternId === internId) {
            const oldAssignedInternName = client.assignedInternName; // Store for history
            ...
            client.changeHistory.push({
                ...
                changes: [{
                    ...
                    newValue: 'Nenhum (Estagiário excluído)'
                }]
            });
        }
    });
